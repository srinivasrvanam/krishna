            ...
            return (
                <div className="w-full h-full">
                  ...
                  <div className="flex flex-wrap justify-center gap-2 mt-2 py-4">
                    {prevVerse && prevVerse.number && <Link to={`/gps/${chapter_num}/${prevVerse.number}`} className="sv-btn fixed bottom-4 left-5">Prev - {prevVerse.number}</Link>}
                    {nextVerse && nextVerse.number && <Link to={`/gps/${chapter_num}/${nextVerse.number}`} className="sv-btn fixed bottom-4 right-5">Next - {nextVerse.number}</Link>}
                  </div>
