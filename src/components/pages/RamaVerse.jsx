    ...
    return (
      <div className="w-full h-full">
        ...
        <div className="flex flex-wrap justify-center gap-2 mt-2 py-4">
          {prevVerse && prevVerse.number && <Link to={`/rama/${chapter_num}/${prevVerse.number}`} className="sv-btn fixed bottom-10 left-5 text-3xl">&lt;{prevVerse.number}</Link>}
          {nextVerse && nextVerse.number && <Link to={`/rama/${chapter_num}/${nextVerse.number}`} className="sv-btn fixed bottom-10 right-5 text-3xl">{nextVerse.number}&gt;</Link>}
        </div>
        <Verse verse={verse} />
        <br />
        <br />
