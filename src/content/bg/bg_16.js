const verses = [
  {
    chapter : 16,
    number  : 1,
    sloka   : [
      'श्रीभगवानुवाच |',
      'अभयं सत्त्वसंशुद्धिर्ज्ञानयोगव्यवस्थिति: |',
      'दानं दमश्च यज्ञश्च स्वाध्यायस्तप आर्जवम् || 1 ||',
      'अहिंसा सत्यमक्रोधस्त्याग: शान्तिरपैशुनम् |',
      'दया भूतेष्वलोलुप्त्वं मार्दवं ह्रीरचापलम् || 2 ||',
      'तेज: क्षमा धृति: शौचमद्रोहोनातिमानिता |',
      'भवन्ति सम्पदं दैवीमभिजातस्य भारत || 3 ||'
    ],
    vicheda : [
      'श्री-भगवान् उवाच |',
      'अभयं सत्त्व-सम्शुद्धिः ज्ञान-योग-व्यवस्थितिः |',
      'दानं दमः च यज्ञः च स्वाध्यायः तप आर्जवम् || 1 ||',
      'अहिंसा सत्यम् अक्रोधः त्याग: शान्तिः अपैशुनम् |',
      'दया भूतेषु अलोलुप्त्वं मार्दवं ह्रीः अचापलम् || 2 ||',
      'तेज: क्षमा धृति: शौचम् अद्रोहः अनातिमानिता |',
      'भवन्ति सम्पदं दैवीम् अभिजातस्य भारत || 3 ||'
    ],
    anvaya  : [
      'श्री-भगवान् उवाच |',
      'हे भारत!',
      '- अभयं सत्त्व-सम्शुद्धिः ज्ञान-योग-व्यवस्थितिः',
      '- दानं दमः च यज्ञः च स्वाध्यायः तप आर्जवम्',
      '- अहिंसा सत्यम् अक्रोधः त्याग: शान्तिः अपैशुनम्',
      '- दया भूतेषु अलोलुप्त्वं मार्दवं ह्रीः अचापलम्',
      '- तेज: क्षमा धृति: शौचम् अद्रोहः अनातिमानिता',
      'अभिजातस्य दैवीम् सम्पदं भवन्ति',
    ],
    transl  : [
      "The Supreme Divine Personality said: O scion of Bharata, these are the saintly virtues of those endowed with a divine nature — fearlessness, purity of mind, steadfastness in spiritual knowledge, charity, control of the senses, sacrifice, study of the sacred books, austerity, and straightforwardness; non-violence, truthfulness, absence of anger, renunciation, peacefulness, restraint from fault-finding, compassion toward all living beings, absence of covetousness, gentleness, modesty, and lack of fickleness; vigor, forgiveness, fortitude, cleanliness, bearing enmity toward none, and absence of vanity(pride)." 
    ],
    notes   : [
      'श्रीभगवान् = the Blessed Lord',
      'उवाच = said',
      'अभयम् = fearlessness',
      'सत्त्व-सम्शुद्धिः = purity of mind (purity of inner being)',
      'ज्ञान-योग-व्यवस्थितिः = steadfastness in the yoga of knowledge',
      'दानम् = charity',
      'दमः = control of the senses',
      'यज्ञः = sacrifice',
      'स्वाध्यायः = study of the scriptures',
      'तपः = austerity',
      'आर्जवम् = honesty / uprightness',
      'अहिंसा = nonviolence',
      'सत्यम् = truthfulness',
      'अक्रोधः = absence of anger',
      'त्यागः = renunciation',
      'शान्तिः = peace',
      'अपैशुनम् = absence of fault-finding (non-slander)',
      'दया = compassion',
      'भूतेषु = towards beings',
      'अलोलुप्त्वम् = non-covetousness / non-greed',
      'मार्दवम् = gentleness',
      'ह्रीः = modesty',
      'अचापलम् = absence of fickleness',
      'तेजः = vigor',
      'क्षमा = forgiveness',
      'धृतिः = fortitude / determination',
      'शौचम् = cleanliness',
      'अद्रोहः = absence of malice (hatred)',
      'अनातिमानिता = absence of pride',
      'भवन्ति = arise / belong',
      'सम्पदम् = qualities / wealth',
      'दैवीम् = divine',
      'अभिजातस्य = of one born (with those tendencies)',
      'भारत = O descendant of Bharata (Arjuna)'
    ],
  },
  {
    chapter : 16,
    number  : 4,
    sloka   : [
      'दम्भो दर्पोऽभिमानश्च क्रोध: पारुष्यमेव च |',
      'अज्ञानं चाभिजातस्य पार्थ सम्पदमासुरीम् || 4 ||'
    ],
    vicheda : [
      'दम्भः दर्पः अभिमानः च क्रोधः पारुष्यम् एव च |',
      'अज्ञानं च अभिजातस्य पार्थ सम्पदम् आसुरीम् ||'
    ],
    anvaya  : [
      'हे पार्थ!',
      '- दम्भः, दर्पः, अभिमानः, क्रोधः, पारुष्यम्, अज्ञानं च',
      'अभिजातस्य आसुरीम् सम्पदं (भवन्ति)',
    ],
    transl  : [
      "Hypocrisy, arrogance, pride, anger, harshness, and ignorance — these, O Arjuna, are the qualities of one born with a demoniac nature." 
    ],
    notes   : [
      'दम्भः = hypocrisy',
      'दर्पः = arrogance',
      'अभिमानः = pride',
      'च = and',
      'क्रोधः = anger',
      'पारुष्यम् = harshness',
      'एव = indeed',
      'च = and',
      'अज्ञानम् = ignorance',
      'च = and',
      'अभिजातस्य = of one born (with such traits)',
      'पार्थ = O Arjuna',
      'सम्पदम् = qualities / disposition',
      'आसुरीम् = demoniac'
    ],
  },
  {
    chapter : 16,
    number  : 5,
    sloka   : [
      'दैवी सम्पद्विमोक्षाय निबन्धायासुरी मता |',
      'मा शुच: सम्पदं दैवीमभिजातोऽसि पाण्डव || 5 ||'
    ],
    vicheda : [
      'दैवी सम्पत् विमोक्षाय निबन्धाय आसुरी मता |',
      'मा शुच: सम्पदं दैवीम् अभिजातः असि पाण्डव ||',
    ],
    anvaya  : [
      'हे पाण्डव !',
      '- (दैवी सम्पत् विमोक्षाय), (आसुरी (सम्पत्) निबन्धाय) मता',
      '(त्वं) दैवीम् सम्पदं अभिजातः असि',
      'मा शुच: (अस्तु)',
    ],
    transl  : [
      "The divine qualities are considered to lead to liberation, while the demoniac qualities lead to bondage. Do not grieve, O Arjuna, for you are born with divine qualities." 
    ],
    notes   : [
      'दैवी = divine',
      'सम्पत् = qualities / disposition',
      'विमोक्षाय = for liberation',
      'निबन्धाय = for bondage',
      'आसुरी = demoniac',
      'मता = is considered',
      'मा = do not',
      'शुचः = grieve',
      'सम्पदं = the qualities',
      'दैवीम् = divine',
      'अभिजातः = born with',
      'असि = you are',
      'पाण्डव = O son of Pandu (Arjuna)'
    ],
  },
  {
    chapter : 16,
    number  : 21,
    sloka   : [
      'त्रिविधं नरकस्येदं द्वारं नाशनमात्मन: |',
      'काम: क्रोधस्तथा लोभस्तस्मादेतत्त्रयं त्यजेत् || 21 ||'
    ],
    vicheda : [
      'त्रिविधं नरकस्य इदम् द्वारं नाशनम् आत्मनः |',
      'काम: क्रोधः तथा लोभः तस्मात् एतत् त्रयम् त्यजेत् ||'
    ],
    anvaya  : [
      'इदम् त्रिविधम् द्वारम् नरकस्य आत्मनः नाशनम् ।',
      'कामः, क्रोधः, तथा लोभः ।',
      'तस्मात् एतत् त्रयम् त्यजेत् ।',
    ],
    transl  : [
      "This is the threefold gate of hell, destructive of the self—lust, anger, and greed. Therefore, one should abandon these three." 
    ],
    notes   : [
      'त्रिविधम् = threefold',
      'नरकस्य = of hell',
      'इदम् = this',
      'द्वारम् = gate / door',
      'नाशनम् = destructive',
      'आत्मनः = of the soul / self',
      'कामः = lust / desire',
      'क्रोधः = anger',
      'तथा = and also',
      'लोभः = greed',
      'तस्मात् = therefore',
      'एतत् = these',
      'त्रयम् = three',
      'त्यजेत् = one should abandon'
    ],
  },
  {
    chapter : 16,
    number  : 22,
    sloka   : [
      'एतैर्विमुक्त: कौन्तेय तमोद्वारैस्त्रिभिर्नर: |',
      'आचरत्यात्मन: श्रेयस्ततो याति परां गतिम् || 22 ||'
    ],
    vicheda : [
      'एतै: विमुक्त: कौन्तेय तमः-द्वारैः त्रिभिः नरः |',
      'आचरति आत्मनः श्रेयः ततः याति परां गतिम् ||'
    ],
    anvaya  : [
      'हे कौन्तेय !',
      'एतै: त्रिभिः तमः-द्वारैः विमुक्त: नरः, आत्मनः श्रेयः आचरति ।',
      'ततः परां गतिम् याति ।',
    ],
    transl  : [
      "O son of Kunti, a man who is freed from these three gates of darkness — lust, anger, and greed — acts for the good of the soul and thereby attains the supreme goal." 
    ],
    notes   : [
      'एतैः = from these',
      'विमुक्तः = freed',
      'कौन्तेय = O son of Kunti',
      'तमः-द्वारैः = gates of darkness (hell)',
      'त्रिभिः = three',
      'नरः = man',
      'आचरति = practices / pursues',
      'आत्मनः = of the self',
      'श्रेयः = good / ultimate welfare',
      'ततः = then / thereby',
      'याति = attains',
      'पराम् = supreme',
      'गतिम् = goal / destination'
    ],
  },
  {
    chapter : 16,
    number  : 23,
    sloka   : [
      'य: शास्त्रविधिमुत्सृज्य वर्तते कामकारत: |',
      'न स सिद्धिमवाप्नोति न सुखं न परां गतिम् || 23||'
    ],
    vicheda : [
      'य: शास्त्रविधिम् उत्सृज्य वर्तते कामकारत: |',
      'न सः सिद्धिम् अवाप्नोति न सुखं न परां गतिम् ||'
    ],
    anvaya  : [
      'य: शास्त्रविधिम् उत्सृज्य कामकारत: वर्तते |',
      'सः न (सिद्धिम् / सुखं / परां गतिम्) अवाप्नोति |',
    ],
    transl  : [
      "He who acts according to his desires, abandoning the injunctions of the scriptures, does not attain perfection, nor happiness, nor the supreme goal." 
    ],
    notes   : [
      'यः = who',
      'शास्त्रविधिम् = scriptural injunctions',
      'उत्सृज्य = disregarding / abandoning',
      'वर्तते = acts / behaves',
      'कामकारतः = according to desire / out of lust',
      'न = not',
      'सः = he',
      'सिद्धिम् = perfection / success',
      'अवाप्नोति = attains',
      'सुखम् = happiness',
      'पराम् = supreme',
      'गतिम् = goal / destination'
    ],
  },
];

export default verses;